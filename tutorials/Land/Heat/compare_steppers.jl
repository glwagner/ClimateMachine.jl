...
for α in α_all
    ...
    global z_coord = get_z(driver_config.grid, z_scale)
    ...
    for solver_type in solver_types
        ...
        all_data[case...] = Dict[dict_of_nodal_states(solver_config, [z_key])]  # store initial condition at ``t=0``
        ...
        callback =
            GenericCallbacks.EveryXSimulationTime(every_x_simulation_time) do
                push!(
                    all_data[case...],
                    dict_of_nodal_states(solver_config, [z_key]),
                )
        ...
        push!(all_data[case...], dict_of_nodal_states(solver_config, [z_key]))
        push!(time_data[case...], gettime(solver_config.solver))
    end # time-steppers
