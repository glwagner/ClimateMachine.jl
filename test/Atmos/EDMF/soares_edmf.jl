...
include(joinpath(clima_dir, "experiments", "AtmosLES", "soares_model.jl"))
...
function main(::Type{FT}) where {FT}
    ...
    soares_args = ArgParseSettings(autofix_names = true)
    add_arg_group!(soares_args, "SOARES")
    @add_arg_table! soares_args begin
        "--surface-flux"
    ...
    cl_args =
        ClimateMachine.init(parse_clargs = true, custom_clargs = soares_args)
    ...
    model =
        soares_model(FT, config_type, zmax, surface_flux; turbconv = turbconv)
...
include(joinpath(
    ...
    "Atmos",
    "EDMF",
    "soares_edmf_regression_test.jl",
))
