...
and Jia et al. (2020).
The key predictents of the EDMF parameterization are the SGS vertical fluxes of heat moisture
...
and or downdafts. The parameterization solves prognostic equations
for first and second moments in the subdomains to provides both the
abovementioned SGS vertical flux and cloud fraction in the host model grid.
...
The EDMF equations take the form of advection-diffusion equations. The sub domain area fraction is given by the mass continuity equation in the $i$th subdomain ($\aSDi{a}$):
